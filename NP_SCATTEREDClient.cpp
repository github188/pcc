...
private:
	...
	static TCPSError Wrap_AddMoudle(PCC_Scatter_RC*, PCC_Scatter*, iscm_PeerCallFlags, const BYTE*&, INT_PTR&, iscm_IRPCOutfiter*) callback;
	...
	static TCPSError Wrap_ListModules(PCC_Scatter_RC*, PCC_Scatter*, iscm_PeerCallFlags, const BYTE*&, INT_PTR&, iscm_IRPCOutfiter*) callback;
				...
				) callback
{
	...
	if(NULL == s_cbMap)
	{
		...
		if(NULL == s_cbMap)
		{
			...
			VERIFY(cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("AddMoudle"), CPtrStrA(s_PCC_Scatter_AddMoudle_TypeInfo_, s_PCC_Scatter_AddMoudle_TypeInfo_len_))).second);
			...
			VERIFY(cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("ListModules"), CPtrStrA(s_PCC_Scatter_ListModules_TypeInfo_, s_PCC_Scatter_ListModules_TypeInfo_len_))).second);
...
void PCC_Scatter_RC::InitCallbackMap_()
{
	...
	VERIFY(cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("PCC_Scatter::AddMoudle"), CallbackSite_(&PCC_Scatter_RC::Wrap_AddMoudle, false))).second);
	...
	VERIFY(cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("PCC_Scatter::ListModules"), CallbackSite_(&PCC_Scatter_RC::Wrap_ListModules, false))).second);
...
TCPSError PCC_Scatter_RC::Wrap_AddMoudle(
				IN PCC_Scatter_RC* thisObj /*= NULL*/,
				IN PCC_Scatter* faceObj /*= NULL*/,
				IN iscm_PeerCallFlags peerCallFlags,
				IN OUT const BYTE*& ptrInParams,
				...
				) callback
{
	...
	IN PCC_ModuleIndex moduleIndex_wrap;
		GET_BASETYPE_EX_(thisObj, ptrInParams, ptrInParamsLen, moduleIndex_wrap.moduleKey);
			GET_STRING_EX_(thisObj, ptrInParams, ptrInParamsLen, moduleIndex_wrap.moduleTag.name);
			GET_BASETYPE_EX_(thisObj, ptrInParams, ptrInParamsLen, moduleIndex_wrap.moduleTag.version);
	...
	if(0 != ptrInParamsLen)
	{
		// NPLogError(("PCC_Scatter::AddMoudle() callback, Malformed request"));
		if(thisObj)
			thisObj->OnNetworkMalformed_();
		return TCPS_ERR_MALFORMED_REQ;
	}
...
TCPSError PCC_Scatter_RC::Wrap_ListModules(
				IN PCC_Scatter_RC* thisObj /*= NULL*/,
				IN PCC_Scatter* faceObj /*= NULL*/,
				IN iscm_PeerCallFlags peerCallFlags,
				IN OUT const BYTE*& ptrInParams,
				...
				) callback
{
	...
	if(0 != ptrInParamsLen)
	{
		// NPLogError(("PCC_Scatter::ListModules() callback, Malformed request"));
		if(thisObj)
			thisObj->OnNetworkMalformed_();
		return TCPS_ERR_MALFORMED_REQ;
	}
	...
	try
	{
		...
		{
			iscm_ClientThreadIDs::AutoThisTID autoCallingFlag(thisObj->m_callingOutTIDs, iscm_cct_callback);
			errTcps = thisObj->ListModules(
				opWrapper->modulesIndex
				);
		}
		else
		{
			ASSERT(faceObj);
			errTcps = faceObj->ListModules(
				opWrapper->modulesIndex
				);
		}
	}
	...
	// OUT tcps_Array<PCC_ModuleIndex> modulesIndex
	OUT const tcps_Array<PCC_ModuleIndex>& modulesIndex_wrap = opWrapper->modulesIndex;
	Set_BaseType_(outfiter, modulesIndex_wrap.LenRef());
	for(int idx1=0; idx1<modulesIndex_wrap.Length(); ++idx1)
	{
		const PCC_ModuleIndex& ref1 = modulesIndex_wrap[idx1];
				...
				)
{
	...
	typeInfo = NULL;
	infoLen = 0;
	err = this->GetStreamedCallbackTypeInfo_("AddMoudle", 9, typeInfo, infoLen);
	matched = (TCPS_OK==err && s_PCC_Scatter_AddMoudle_TypeInfo_len_==infoLen && 0==strncmp(s_PCC_Scatter_AddMoudle_TypeInfo_, typeInfo, infoLen));
	VERIFY(m_streamedCallbackMap.insert(std::make_pair(CPtrStrA("AddMoudle", 9), matched)).second);
	if(typeInfo)
		tcps_Free(typeInfo);
	...
	typeInfo = NULL;
	infoLen = 0;
	err = this->GetStreamedCallbackTypeInfo_("ListModules", 11, typeInfo, infoLen);
	matched = (TCPS_OK==err && s_PCC_Scatter_ListModules_TypeInfo_len_==infoLen && 0==strncmp(s_PCC_Scatter_ListModules_TypeInfo_, typeInfo, infoLen));
	VERIFY(m_streamedCallbackMap.insert(std::make_pair(CPtrStrA("ListModules", 11), matched)).second);
	if(typeInfo)
		tcps_Free(typeInfo);
				...
				)
{
	...
	if(NULL == s_cbMap)
	{
		...
		if(NULL == s_cbMap)
		{
			...
			cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("AddMoudle"), FuncPair(CPtrStrA(s_PCC_Scatter_AddMoudle_TypeInfo_, s_PCC_Scatter_AddMoudle_TypeInfo_len_), (PROC)&Local_AddMoudle)));
			...
			cbMap.insert(std::make_pair(CONST_STR_TO_PTRSTR_A("ListModules"), FuncPair(CPtrStrA(s_PCC_Scatter_ListModules_TypeInfo_, s_PCC_Scatter_ListModules_TypeInfo_len_), (PROC)&Local_ListModules)));
			s_cbMap = &cbMap;
		}
	}
...
TCPSError PCC_Scatter::Local_AddMoudle(
				IN void* sessionObj,
				...
				) callback
{
	...
	if(TCPS_ERR_STREAMED_CALLBACK_NOT_IMPLEMENTED != err)
	{
		if(!pCC_ScatterObj_wrap->IsStreamedCallbackMatched_("AddMoudle", 9))
			return TCPS_ERR_CALLBACK_NOT_MATCH;

		DataOutfiter dataOutfiter;

		// IN PCC_ModuleIndex moduleIndex
			Put_BaseType_(&dataOutfiter, moduleIndex_wrap.moduleKey);
				Put_String_(&dataOutfiter, moduleIndex_wrap.moduleTag.name.Get(), moduleIndex_wrap.moduleTag.name.LenRef());
				Put_BaseType_(&dataOutfiter, moduleIndex_wrap.moduleTag.version);
	...
	return pCC_ScatterObj_wrap->AddMoudle(
					moduleIndex_wrap,
					moudleFiles_wrap
					);
}
...
TCPSError PCC_Scatter::Local_ListModules(
				IN void* sessionObj,
				...
				) callback
{
	...
	if(TCPS_ERR_STREAMED_CALLBACK_NOT_IMPLEMENTED != err)
	{
		if(!pCC_ScatterObj_wrap->IsStreamedCallbackMatched_("ListModules", 11))
			return TCPS_ERR_CALLBACK_NOT_MATCH;
		...
		if(TCPS_OK == err)
		{
			...
			// OUT tcps_Array<PCC_ModuleIndex> modulesIndex
			PICK_BASETYPE_Q_EX(pCC_ScatterObj_wrap, iscm_replied_picker, array_len);
			modulesIndex_wrap.Resize(array_len);
			for(int idx3=0; idx3<modulesIndex_wrap.Length(); ++idx3)
			{
				PCC_ModuleIndex& ref3 = modulesIndex_wrap[idx3];
					PICK_BASETYPE_Q_EX(pCC_ScatterObj_wrap, iscm_replied_picker, ref3.moduleKey);
						PICK_STRING_Q_EX(pCC_ScatterObj_wrap, iscm_replied_picker, ref3.moduleTag.name);
						PICK_BASETYPE_Q_EX(pCC_ScatterObj_wrap, iscm_replied_picker, ref3.moduleTag.version);
			}
	...
	return pCC_ScatterObj_wrap->ListModules(
					modulesIndex_wrap
					);
}
				...
				) method
{
	...
	// IN PCC_ModuleProperty moduleProperty
		...
		Put_BaseType_(&m_dataOutfiter, moduleProperty_wrap.cpuType);
		Put_BaseType_(&m_dataOutfiter, moduleProperty_wrap.osType);
		Put_BaseType_(&m_dataOutfiter, moduleProperty_wrap.executeBits);
		Put_BaseType_(&m_dataOutfiter, moduleProperty_wrap.binaryType);
		...
		for(int idx2=0; idx2<modulesInfo_wrap.Length(); ++idx2)
		{
			PCC_ModulePropWithKey& ref2 = modulesInfo_wrap[idx2];
				PICK_BASETYPE_Q(iscm_replied_picker, ref2.key);
					...
					PICK_BASETYPE_Q(iscm_replied_picker, ref2.prop.cpuType);
					PICK_BASETYPE_Q(iscm_replied_picker, ref2.prop.osType);
					PICK_BASETYPE_Q(iscm_replied_picker, ref2.prop.executeBits);
					PICK_BASETYPE_Q(iscm_replied_picker, ref2.prop.binaryType);
		}
		if(iscm_replied_picker != iscm_replied_end)
		{
